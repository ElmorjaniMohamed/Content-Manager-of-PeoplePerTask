...
<body class="bg-body-bg bg-cover bg-center">
        ...
        <li>
          <ul class="flex justify-center items-center gap-4">
            ...
            <li class="hidden ht:flex justify-center items-center gap-3 flex-row">
              <button type="button"
                class="flex text-defaultText rounded-full bg-loginBtnBg py-2 px-8 justify-center items-center font-poppins text-base font-medium w-[full]"><a
  ...
  <main>
    <div
      ...
      <h1 class="text-4xl text-center font-bold font-fredoka dark:text-white">Sign Up to <span
          class="text-mainBlue dark:text-mainPurple">PeoplePerTask</span></h1>
      <form id="registrationForm" action="script.php" method="POST" class="mt-10 mb-4" onsubmit="return validateForm()">
          ...
          <div class="flex flex-col items-center justify-between space-y-5 w-[100%]">
            ...
            <label for="username" class="w-[100%]">
              <input id="username" name="username" type="text"
                ...
                placeholder="Username" required oninput="clearMessages()">
            ...
            <label for="email" class="w-[100%]">
              <input id="email" name="email" type="email"
                class="w-[100%] py-3 border-gray-300 border-2 rounded-lg px-3 focus:outline-none focus:border-mainBlue dark:focus:border-mainPurple"
                placeholder="Email" required oninput="clearMessages()">
  ...
  <script>
    function validateForm() {
      ...
      if (username.trim() === '') {
        displayErrorMessage('Please enter a username', 'usernameError');
        return false;
      } else if (!isValidUsername(username)) {
        displayErrorMessage('Only letters and white space allowed', 'usernameError')
        return false
      } else {
        displaySuccessMessage('Username is valid', 'usernameSuccess');
      }

      if (email.trim() === '') {
        displayErrorMessage('Please enter an email', 'emailError');
        return false;
      } else {
        displaySuccessMessage('Email is valid', 'emailSuccess');
      }
      ...
      if (password.trim() === '') {
        ...
        displayErrorMessage('Password must be 8 characters or longer and contain at least one uppercase letter, one lowercase letter, and one digit.', 'passwordError');
        return false;
      } else {
        displaySuccessMessage('Password is valid', 'passwordSuccess');
      }

      // Additional validation logic can be added here

      // If all validations pass, display success message and return true to submit the form
      displaySuccessMessage('Form submitted successfully!', 'formSuccess');
      showSubmitPopup();
      return true;
    }

    function isValidPassword(password) {
      // Password must be 8 characters or longer and contain at least one uppercase letter, one lowercase letter, and one digit
    ...
    function displaySuccessMessage(message, fieldId) {
      ...
      successMessageSpan.classList.add('success-message');
    }

    function clearMessages() {
      var messageElements = document.querySelectorAll('.error-message, .success-message');
